...
function runDailyOrchestration() {
  ...
  try {
    ...
    // Step 6b: Store performance data for seating analysis
    results.steps.push({
      step: 'Seating Data Bridge',
      status: 'started',
      timestamp: new Date().toISOString()
    });
    const seatingDataResults = updateSeatingPerformanceData(aggregatedData, hubConfig);
    results.steps[results.steps.length - 1].status = 'complete';
    results.steps[results.steps.length - 1].periodsUpdated = seatingDataResults.periodsUpdated;
